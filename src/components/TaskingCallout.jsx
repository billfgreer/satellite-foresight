...
import StacItemModal from './StacItemModal.jsx'
...
// The whole point of this section: if it's not delivered yet, get the user to
// the moment they'll have access (and a way to be told when that happens); if
// it's delivered, get them straight to the catalog record.
function AccessSection({ task, formatTime, onOpenStac }) {
  ...
  if (task.status === 'Delivered') {
    return (
      <div className={styles.accessBox}>
        <div className={styles.accessHeadline}>✓ Delivered — imagery is available now</div>
        <button className={styles.stacBtn} onClick={onOpenStac}>
  ...
  const availableAt = task.timestamps.availableAt
  return (
    <div className={styles.accessBox}>
      <div className={styles.accessHeadline}>
        {availableAt
          ? <>Available <strong>{relativeFromNow(availableAt)}</strong> — {formatTime(availableAt)}</>
          : 'Availability not yet estimated — depends on capture and processing'}
      </div>
...
export default function TaskingCallout({ task, onClose }) {
  const [showStac, setShowStac] = useState(false)
  ...
  return (
    <>
        ...
        <div className={styles.overlay} onClick={onClose}>
          <div className={styles.modal} onClick={e => e.stopPropagation()}>
            ...
            <div className={styles.body}>
              ...
              <AccessSection task={task} formatTime={formatTime} onOpenStac={() => setShowStac(true)} />
      ...
      {showStac && <StacItemModal task={task} onClose={() => setShowStac(false)} />}
    </>
  )
}
